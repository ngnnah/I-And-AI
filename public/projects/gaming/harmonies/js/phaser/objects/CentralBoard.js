 ...
 *
 * Displays token supply spaces (3 for solo, 5 for multiplayer)
 */

export default class CentralBoard extends Phaser.GameObjects.Container {
  constructor(scene, x, y, isSolo = false) {
    super(scene, x, y);

    this.isSolo = isSolo;
    this.numSpaces = isSolo ? 3 : 5; // Solo=3, Multiplayer=5
    this.spaces = [];
    this.spaceSize = 120;
    this.spacing = 20;
    ...
    console.log('[CentralBoard] Created with', this.numSpaces, 'token supply spaces', isSolo ? '(SOLO MODE)' : '(MULTIPLAYER)');
  }

  createSpaces() {
    // Create spaces (3 for solo, 5 for multiplayer)
   ...
   * @param {number} spaceIndex - 0-2 (solo) or 0-4 (multiplayer)
   * @param {Array} tokens - Array of {color: "blue", ...}
   */
  updateSpace(spaceIndex, tokens) {
    if (spaceIndex < 0 || spaceIndex >= this.numSpaces) return;
